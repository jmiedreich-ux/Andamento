...
test('validates repository roots and stores only canonical local Git repositories', async t => {
  ...
  assert.deepEqual(fixture.service.verifyInvariants().passed.length, 16);
});
...
test('persists the planning loop on disk across a real application restart in WAL mode', async t => {
  ...
  assert.equal(Number(fixture.database.prepare('SELECT COUNT(*) AS count FROM schema_migrations').get().count), 4);
  await fixture.close();
  await fixture.open();
...
test('prepares and saves 100 accepted inputs but atomically refuses 101', async t => {
  ...
  assert.equal(fixture.service.verifyInvariants().passed.length, 16);
});
