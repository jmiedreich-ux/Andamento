...
test('validates repository roots and stores only canonical local Git repositories', async t => {
  ...
  assert.deepEqual(fixture.service.verifyInvariants().passed.length, 21);
});
...
test('persists the planning loop on disk across a real application restart in WAL mode', async t => {
  ...
  assert.equal(Number(fixture.database.prepare('SELECT COUNT(*) AS count FROM schema_migrations').get().count), 5);
  await fixture.close();
  await fixture.open();
